...
async function main() {
  ...
  const force = !isProd || process.env.FORCE_SEED === 'true';
  ...
  if (!force) {
    ...
    if (userCount > 0 || sectorCount > 0 || packageCount > 0 || productCount > 0) {
      console.log('ℹ️ Database already contains data. Skipping auto-seed in production.');
      return;
    }
  }

  console.log('🚀 Starting native database seeding...');
  ...
  for (const pkg of packages) {
    ...
    if (!existing) {
      await prisma.servicePackage.create({ data: { ...pkg, id } });
    } else {
      // Safe update of the image if it is missing or placeholder
      if (!existing.image || existing.image.includes('placeholder') || existing.image.includes('picsum.photos')) {
  ...
  for (const prod of products) {
    ...
    if (!existing) {
      await prisma.product.create({ data: { ...prod, id } });
    } else {
      // Safely update image and category if missing or old placeholder/categories
      const updates = {};
      if (!existing.image || existing.image.includes('placeholder') || existing.image.includes('picsum.photos')) {
        updates.image = prod.image;
      }
