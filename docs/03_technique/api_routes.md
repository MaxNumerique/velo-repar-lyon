...
Ces routes sont accessibles sans être connecté. Elles alimentent le wizard de réservation client.
...
3. Envoi d'un email de confirmation au client (Nodemailer).
4. Envoi d'une notification Web Push au technicien.
...
## 2. Routes Authentifiées — Interventions
...
Webhook Clerk : déclenché automatiquement par Clerk lors de la création d'un nouveau compte. Synchronise le profil utilisateur Clerk dans la table `User` PostgreSQL locale.
