...
export async function POST(req) {
  try {
    ...
    if (clerkErrorCode === 'form_password_length_too_short') {
      ...
      message = 'Ce mot de passe est trop commun et a été compromis dans une fuite de données. Veuillez en choisir un autre.'
    } else if (clerkErrorCode === 'form_data_missing' && error.errors?.[0]?.longMessage?.includes('username')) {
      message = 'Le système requiert un nom d\'utilisateur (username) qui est manquant ou invalide.'
